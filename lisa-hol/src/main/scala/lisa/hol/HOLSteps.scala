...
object HOLSteps extends lisa._HOL {
  ...
  val betaConv = Theorem(((x :: A), tforall(x :: A, t(x) :: B)) |- holeq(B) * (abs(A)(t) * x) * t(x)) {
    ...
    val txb = have(x :: A |- t(x) :: B) by InstantiateForall(x)(lastStep)
        ...
        case (HOLSequent(_, _, *(*(=:= #@ (aa), s), ta)), HOLSequent(_, _, *(*(=:= #@ (ab), tb), u))) => // equality is too strict
          if isSame(ta, tb) then
            if isSame(aa, ab) then
              ...
              val r0 = have(((s :: aa), (ta :: aa), (u :: aa), (holeq(aa) * ta * u) === One) |- (holeq(aa) * s * u) === One) by Cut
                .withParameters(holeq(aa) * s * ta)(p0, eqTrans of (x := s, y := ta, z := u, A := aa))
        ...
        case (HOLSequent(_, _, (=:= #@ typ1) * ff * gg), HOLSequent(_, _, (=:= #@ typ2) * xx * yy)) => // equality is too strict
          typ1 match {
            case ->:(inner, b) if isSame(typ2, inner) => // this CANNOT use equality because of alpha equivalence
              ...
              val s1 = have((xx :: typ2, yy :: typ2, ff :: typ1, gg :: typ1, ff =:= gg, xx =:= yy, ∃(x, x ∈ typ2), ∃(x, x ∈ b)) |- (ff * xx =:= gg * yy)) by Weakening(
                mk_comTHM of (f := ff, g := gg, x := xx, y := yy, A := typ2, B := b)
              )
  ...
  object _ETA {
    def apply(using proof: Proof)(x: TypedVariable, t: Expr[Ind]): ProofJudgement = Subproof { ip ?=>
      ...
      val s1 =
        have((t :: ttype, x :: x.typ, ∃(x, x :: x.typ), ∃(x, x :: restype)) |- holeq(ttype) * (fun(x :: x.typ, t * x)) * t) by Weakening(etaConv of (HOLSteps.x := x, f := t, A := x.typ, B := restype))
  ...
  object _INST {
    def apply(using proof: Proof)(inst: Seq[(Variable[Ind], Expr[Ind])], prem: Thm): ProofJudgement = Subproof { ip ?=>
      ...
      instWithProofs.foldLeft(h0: Thm) { case (h, (v, t, typProof)) =>
  ...
  object HOLProofType {
    ...
    private val cache: mutable.Map[Long, Thm] = mutable.Map.empty
  ...
  object Clean {
    ...
    def allComposites(using proof: Proof)(prem: Thm): ProofJudgement = Subproof { ip ?=>
      ...
      val assumedOccurrents = prem.statement.left.collect { case exists(x, y ∈ ty) =>
        ty
      }
