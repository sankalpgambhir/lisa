...
object VarsAndFunctions /*extends lisa.Main*/:
  ...
  class HOLPolymorphicType[S: Sort](id: Identifier, val freeTypeVars: Seq[TypeVariable], val nonEmptyThm: Thm) extends Constant[S](id)

  type HOLConstantType = HOLPolymorphicType[Ind]
  object HOLConstantType:
    def apply(id: Identifier, nonEmptyThm: Thm): HOLConstantType =
  ...
  class HOLConstant(id: Identifier, override val typ: Expr[Ind], val thm: Thm) extends TypedConstant(id, typ, thm):
    val holDefinition =
      ...
      Subproof {
        val s1 = have((left :: typ) |- holeq(typ) * left * left) by Weakening(eqRefl of (A := typ, x := left, y := left))
        val s2 = have(holeq(typ) * left * left) by Cut(thm, s1)
        val s3 = have(left === right |- holeq(typ) * left * right) by RightSubstEq.withParameters(Seq((left, right)), (Seq(x), holeq(typ) * left * x))(s2)
        have(holeq(typ) * left * right) by Cut(theDefinition, s3)
      }.destruct._1
  class HOLPolymorphicConstant[S: Sort](id: Identifier, override val typ: FunctionalClass, override val justif: Thm) extends TypedConstantFunctional[S](id, typ, justif):
    /**
    ...
    val holDefinition =
      ...
      val otyp = typ.outTyp
      Subproof {
        val typeJustif = have(nonEmpties |- (left :: otyp)) by InstantiateForall(typ.args*)(justif)
        val s1 = have((left :: otyp) |- holeq(otyp) * left * left) by Restate.from(eqRefl of (A := otyp, x := left, y := left))
        val s2 = have(nonEmpties |- holeq(otyp) * left * left) by Cut(typeJustif, s1)
        val s3 = have((nonEmpties, left === right) |- holeq(otyp) * left * right) by RightSubstEq.withParameters(Seq((left, right)), (Seq(x), holeq(otyp) * left * x))(s2)
        have(nonEmpties |- holeq(otyp) * left * right) by Cut(theDefinition, s3)
      }.destruct._1
  ...
  def getContext(lo: LisaObject): Set[Expr[Prop]] =
    ...
    frees.flatMap {
      ...
      case v: TypeVariable =>
        val vv = if v.id.name == "x" then Variable[Ind](K.Identifier("x", v.id.no + 1)) else Variable[Ind](K.Identifier("x"))
        Set(exists(vv, vv ∈ v))
      case _ => Set.empty
    }

  def nonEmpty(typ: Variable[Ind]): Expr[Prop] =
    val v = if typ.id.name == "x" then Variable[Ind](K.Identifier("x", typ.id.no + 1)) else Variable[Ind](K.Identifier("x"))
    exists(v, v ∈ typ)

  class LisaHOLException(errorMessage: String)(using sourcecode.Line, sourcecode.File) extends Exception(errorMessage) {
  ...
  class HOLSequent(
    ...
    infix def ++<<(s1: HOLSequent): HOLSequent =
      new HOLSequent(this.premises ++ s1.premises, this.folPremises ++ s1.folPremises, conclusion, typeAssigns ++ s1.typeAssigns, typeVarsNonEmpty ++ s1.typeVarsNonEmpty)
    infix def --<<(s1: HOLSequent): HOLSequent =
      new HOLSequent(this.premises -- s1.premises, this.folPremises -- s1.folPremises, conclusion, typeAssigns -- s1.typeAssigns, typeVarsNonEmpty -- s1.typeVarsNonEmpty)
  ...
  def TypingTheorem(using
      om: lisa.utils.prooflib.OutputManager,
      fullName: sourcecode.FullName,
      name: sourcecode.Name,
      line: sourcecode.Line,
      file: sourcecode.File
  )(assignment: Expr[Prop]): Theorem =
    val contextAssigns = getContext(assignment)
    Theorem(using lib, om, file, line, fullName, name)(contextAssigns |- assignment) {
